...
xds_MJOs_sim = xr.Dataset(
    {
        'RMM1': (('time',), mjoRMM1Sim[0,:]),
        'RMM2': (('time',), mjoRMM2Sim[0,:]),
    },
    coords = {'time': [datetime(r[0],r[1],r[2]) for r in mjoDatesSim]}
)
...
xds_PCs_sim = xr.Dataset(
    ...
    coords = {'time': [datetime(r[0],r[1],r[2]) for r in swtDatesSim[0:len(swtPC1)]]}
)
# reindex annual data to daily data
